...
#endif
#endif
	// Test if the area for the kernel text + module text is
	// greater than the ARM branch (+/- 32Mb)?
	// If so panic
	if(((void *)&_etext - (void *) MODULE_START) > 32*1024*1024)
	  panic("(&_etext - MODULE_START BAD) > 32Mb\n");
}
