...
int mv_nand_correct_data_rs(struct mtd_info *mtd, u_char *data, u_char *store_ecc, u_char *calc_ecc)
{
	...
	/* compare ECC and see if decode is needed */
	for(i=0; i<10; i++) {
		if(store_ecc[i] != calc_ecc[i]) {
			printk("ecc is not equal(0x%x != 0x%x)\n", store_ecc[i],calc_ecc[i]);
			goto correct;
		}
	}

	/* calculated ecc equal to stored ecc - no need to correct */
	return 0;
