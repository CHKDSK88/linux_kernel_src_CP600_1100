...
extern void dsdt_dump_all_switch_stats(int i);
...
void run_com_stats(void) {
	...
	switch(status) {
		...
		case STS_SWITCH_STATS:
			...
			break;
		case STS_SWITCH_INFO:
			dsdt_dump_all_switch_stats(port);
			break;
...
int run_netdev_vtu_cmd(const char *buffer) 
{
	int			scan_count;
	struct net_device* 	dev;

        scan_count = sscanf(buffer, NETDEV_VTU_CMD_STRING, NETDEV_VTU_SCANF_LIST);

       if( scan_count != NETDEV_VTU_LIST_LEN) 
	{
                printk("vtu command bad format %x != %x\n", scan_count, NETDEV_VTU_LIST_LEN );
                return 1;
        }
        ...
        switch(command) 
	{
			...
			mv_gateway_set_vtu(port, tag, 1);
                      break;

                case COM_NETDEV_VTU_DEL:
			mv_gateway_set_vtu(port, tag, 0);
                        break;
#endif /* CONFIG_MV_GATEWAY */
...
int run_net_dev_en_cmd(const char *buffer)
{
	int                     scan_count;

	scan_count = sscanf(buffer, NET_DEV_EN_CMD_STRING, NET_DEV_EN_SCANF_LIST);

	if( scan_count != NET_DEV_EN_LIST_LEN)
        {
                printk("netdev-enable command bad format %x != %x\n", scan_count, NET_DEV_EN_LIST_LEN );
                return 1;
        }
