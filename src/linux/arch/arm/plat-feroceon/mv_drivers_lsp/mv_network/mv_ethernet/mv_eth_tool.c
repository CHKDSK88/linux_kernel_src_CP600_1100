...
extern int mv_eth_start(struct net_device *);
extern int mv_eth_stop(struct net_device *);
		...
		case ETH_SS_STATS:
			...
			for (q = 0; q < MV_ETH_RX_Q_NUM; q++) {
				for (i = 0; i < MV_ETH_TOOL_RX_QUEUE_STATS_LEN; i++) {
					char *str = mv_eth_tool_rx_queue_strings_stats[i].stat_string;
			...
			for (q = 0; q < MV_ETH_TX_Q_NUM; q++) {
				for (i = 0; i < MV_ETH_TOOL_TX_QUEUE_STATS_LEN; i++) {
					char *str = mv_eth_tool_tx_queue_strings_stats[i].stat_string;
					memcpy(p, str, ETH_GSTRING_LEN);
					strcat(p, qnum[q]);
					p += ETH_GSTRING_LEN;
				}
...
int mv_eth_tool_phys_id(struct net_device *netdev, u32 data)
{
	...
	/*ofer: mv_eth_tool_read_phy_reg(priv->phy_id, ETH_TOOL_PHY_LED_CTRL_PAGE,
				ETH_TOOL_PHY_LED_CTRL_REG, &old_led_state);*/
	mvEthPhyRegRead(priv->phy_id, 25, &old_led_state);
	
	/* Force LED blinking (all LED pins) */
	mvEthPhyRegWrite(priv->phy_id, 25, 0x15);
	
	/*ofer: mv_eth_tool_write_phy_reg(priv->phy_id, ETH_TOOL_PHY_LED_CTRL_PAGE,
				ETH_TOOL_PHY_LED_CTRL_REG, 0x0BBB);*/
	msleep_interruptible(data * 1000);
	mvEthPhyRegWrite(priv->phy_id, 25, old_led_state);
