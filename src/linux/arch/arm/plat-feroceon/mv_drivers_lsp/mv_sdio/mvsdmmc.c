...
#define MPP_CONTROL_1_REGISTER 0X10004
#define MPP_CONTROL_2_REGISTER 0X10008

#define SD_IN_OUT_GPIO_INT 46

#define GPP_BIT(gpp)    0x1 << GPP_ID(gpp)
#define GPP_GROUP(gpp)  gpp/32
#define GPP_ID(gpp)     gpp%32

void mv_unmask_irq(unsigned int irq)
{
	if(irq < 32)
	{
	...
	{
		MV_REG_BIT_RESET(MV_GPP_IRQ_HIGH_EDGE_REG , (1 << (irq - 96)) );
...
void mv_clear_irq(int irq)
{
	/* Disable all interrupts initially. */
	if(irq < 32)
	{
		MV_REG_BIT_RESET(MV_IRQ_CAUSE_LOW_REG, (1 << irq) );
	}
	else if(irq < 64)/* (irq > 32 && irq < 64) */
	{
		MV_REG_BIT_RESET(MV_IRQ_CAUSE_HIGH_REG, (1 << (irq - 32)) );
	}
	else if(irq < 96)/* (irq > 64 && irq < 96) */
	{
		MV_REG_BIT_RESET(MV_GPP_IRQ_CAUSE_REG, (1 << (irq - 64)) );
	}
	else if(irq < NR_IRQS)/* (irq > 96 && irq < 128) */
	{
		(MV_REG_READ(MV_GPP_IRQ_HIGH_POLARITY) & (1 << (irq - 96)))?
...
static int mvsdmmc_probe(struct platform_device *pdev)
{
	...
	host->irq_detect =IRQ_GPP_46;
	if (detect){
		mv_unmask_irq(IRQ_GPP_46);
		mvGppTypeSet (GPP_GROUP(SD_IN_OUT_GPIO_INT), GPP_BIT(SD_IN_OUT_GPIO_INT),(MV_GPP_IN & GPP_BIT(SD_IN_OUT_GPIO_INT)));
...
static int __init mvsdmmc_init(void)
{
	...
	if (! is_sd_card_supported())
	{
		printk("SD card is not supported on this platform\n");
		return 1;
	}
	if (is_no_external_ports())
	{
		printk("SD card is disabled\n");
		return 1;
	}
	//init MPP 12-17 to support SD
	MV_REG_BIT_SET(MPP_CONTROL_1_REGISTER, (1 << 16) );
	MV_REG_BIT_SET(MPP_CONTROL_1_REGISTER, (1 << 20) );
	MV_REG_BIT_SET(MPP_CONTROL_1_REGISTER, (1 << 24) );
	MV_REG_BIT_SET(MPP_CONTROL_1_REGISTER, (1 << 28) );

	MV_REG_BIT_SET(MPP_CONTROL_2_REGISTER, (1 ) );
	MV_REG_BIT_SET(MPP_CONTROL_2_REGISTER, (1 << 4) );
	MV_REG_BIT_SET(MPP_CONTROL_2_REGISTER, (1 << 8) );
	MV_REG_BIT_SET(MPP_CONTROL_2_REGISTER, (1 << 12) );
...
static void __exit mvsdmmc_exit(void)
{
	...
	mvGppTypeSet (GPP_GROUP(SD_IN_OUT_GPIO_INT), GPP_BIT(SD_IN_OUT_GPIO_INT),(MV_GPP_OUT & GPP_BIT(SD_IN_OUT_GPIO_INT)));
	mvsdmmc_dbg_exit();
}
