...
GT_BOOL gphyIsFiber(GT_QD_DEV *dev, GT_LPORT  port)
{
	GT_PHY_INFO	 phyInfo;

	if(driverFindPhyInformation(dev, port, &phyInfo) != GT_OK)
	{
	    DBG_INFO(("Unknown PHY device.\n"));
		return GT_FALSE;
	}
	return phyInfo.flag & GT_PHY_FIBER;
}


GT_STATUS gprtPortGetAutoNeg(GT_QD_DEV *dev, GT_LPORT  port, GT_PHY_AUTO_MODE *mode)
{
	GT_BOOL       retVal = 0;         /* Functions return value.      */
	...
	if (!(phyInfo.flag & GT_PHY_GIGABIT))
	{
		retVal = u16Data & QD_PHY_AUTONEGO;
	}

	if(retVal) { //autoneg on, lets check what we are advertising
		if(hwReadPhyReg(dev,hwPort,QD_PHY_AUTONEGO_AD_REG,&u16Data) != GT_OK)
		{
			DBG_INFO(("Not able to read Phy Reg(port:%d,offset:%d).\n",hwPort,QD_PHY_AUTONEGO_AD_REG));
			gtSemGive(dev,dev->phyRegsSem);
