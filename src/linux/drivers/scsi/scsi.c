...
int scsi_dispatch_cmd(struct scsi_cmnd *cmd)
{
	...
	if ((cmd->device->host->hostt->support_scattered_spinup == 1) && /*the flag is requaired in order to support the scattered spinup*/
		(is_scsi_spinup_enabled()))
	{
		if (!in_atomic() && (cmd->device->sdev_power_state == SDEV_PW_SPINNING_UP)) {
			spin_unlock_irqrestore(host->host_lock, flags);
			/* disk is spinning up now! (or waiting for his turn to spinup) all new commands will be queued */
			wait_event(cmd->device->spinup_wq, cmd->device->sdev_power_state != SDEV_PW_SPINNING_UP);
			spin_lock_irqsave(host->host_lock, flags);
		}
		switch (cmd->cmnd[0])
		{
	...
	*/
	if (is_scsi_spinup_enabled())
	{
		...
		if (cmd->device->sdev_power_state == SDEV_PW_SPINNING_UP) 
		{
			...
			/* Waking up the wait queue of the commands waited to the spinup */
			wake_up(&cmd->device->spinup_wq);
		}
		else
			spin_unlock_irqrestore(cmd->device->host->host_lock, flags);
	}
