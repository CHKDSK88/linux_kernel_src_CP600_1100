...
#define SIERRA_NET_TIMER_EXPIRY        0x02
	...
	int (*orig_change_mtu)(struct net_device *dev, int new_mtu);
...
{
	return be32_to_cpu(get_unaligned((const u32*)p));
}

static inline u16 get_unaligned_be16(const void *p)
{
	return be16_to_cpu(get_unaligned((const u16*)p));
}

static inline void put_unaligned_be16(const u16 val, void *p)
{
	put_unaligned(cpu_to_be16(val), (u16*)p);
}
...
static int ipv4_is_multicast(__be32 addr)
{
	return (addr & cpu_to_be32(0xf0000000)) == cpu_to_be32(0xe0000000);
}
...
static u64 get_dns(struct usbnet *dev)
{
	...
	retval = cpu_to_be64(((u64)priv->dns1_addr << 32)|(u64)priv->dns2_addr);
	
	devdbg(dev, "dns 1, 2 = 0x%0x, 0x%0x",priv->dns1_addr, priv->dns2_addr);
	devdbg(dev, "dns 2 & 1 = 0x%llx", retval);
	...
	while (curp < endp) {
		...
		case DHCP_TAG_MESSAGE_TYPE:
			if (*curp == 1) {
				...
				deverr(dev, "Couldn't handle MESSAGE TYPE");
			}
			break;
		...
		case DHCP_TAG_SERVER_ID:
			if (*curp == 4) {
				...
				deverr(dev, "Couldn't handle SERVER_ID");
			}
			break;
		...
		case DHCP_TAG_REQUESTED_IP:
			if (*curp == 4) {
				...
				deverr(dev, "Couldn't handle REQUESTED_IP");
			}
			break;
...
struct sk_buff * handle_dhcp(struct sk_buff * skb, struct usbnet *dev)
{
	...
	//	printk("request->hw_addr=%x:%x:%x:%x:%x:%x dev->net->dev_addr=%x:%x:%x:%x:%x:%x/* if not related to this net interface ... stop processing */\n",
	//	request->hw_addr[0],request->hw_addr[1],request->hw_addr[2],request->hw_addr[3],request->hw_addr[4],request->hw_addr[5],
	//	dev->net->dev_addr[0],dev->net->dev_addr[1],dev->net->dev_addr[2],dev->net->dev_addr[3],dev->net->dev_addr[4],dev->net->dev_addr[5]);
	//		for (i=0;i<6;i++)
	//	dev->net->dev_addr[i] = request->hw_addr[i];
		return NULL;
	}
...
static int sierra_net_parse_lsi(struct usbnet *dev, char *data, int datalen)
{
	...
	if (datalen < sizeof(lsi_umts_t)) {
		deverr(dev, "%s: datalen%d, expected %Zu",
	...
	if (lsi->link_type != SIERRA_NET_AS_LINK_TYPE_IPv4) {
		deverr(dev, "Link unavailable: 0x%02x", lsi->link_type);		
		return 0;
	}
		...
		struct hip_hdr	* hh)
{
	...
	if (sierra_net_reset_lsi)
	{
		printk("Sierra_net: reset due to Invalid LSI\n");
		if ( dev->udev->portnum == 3) 
		
			sw_set_usb_disconnect(1); //USB1
		else
			sw_set_usb_disconnect(2); //USB2
	}
	}
}
...
static void sierra_net_kevent(struct work_struct *work)
{
	...
	if (test_bit(SIERRA_NET_EVENT_RESP_AVAIL, &priv->kevent_flags)) {
		...
		if (unlikely(len < 0)) {
			...
			devdbg(dev, "%s: Received status message, %04x bytes",
				__FUNCTION__, len);
			...
			case SIERRA_NET_HIP_LSI_UMTSID:
				devdbg(dev, "LSI for ctx:%d", 
					hh.msgspecific.byte );
				
				sierra_net_handle_lsi(dev, buf, &hh);
				break;
			case SIERRA_NET_HIP_RESTART_ID:
				devdbg(dev, "Restart reported: %d, stopping sync timer",
						hh.msgspecific.byte);
				...
				clear_bit(SIERRA_NET_TIMER_EXPIRY, 
					  &priv->kevent_flags);
/*				buf[len-1] = '\0';
*				devdbg(dev, "->%s<-", &buf[hh.hdrlen]);
*/
				break;
			case SIERRA_NET_HIP_HSYNC_ID:		
				devdbg(dev, "SYNC received");
				sierra_net_send_sync(dev);
				break;
			...
			case SIERRA_NET_HIP_RCGI:
				...
				break;
			default:
				deverr(dev,"Unrecognized HIP msg, msgid 0x%02x", 
					hh.msgid.byte);
				break;
			}
		}
	...
	/* The sync timer bit might also be set */
	if (test_bit(SIERRA_NET_TIMER_EXPIRY, &priv->kevent_flags)) {
		clear_bit(SIERRA_NET_TIMER_EXPIRY, &priv->kevent_flags);
		devdbg(dev, "Deferred sync timer expiry");
		sierra_net_dosync( priv->usbnet );
	}

	if (priv->kevent_flags)
		devdbg(dev, "sierra_net_kevent done, kevent_flags = 0x%lx",
			priv->kevent_flags);
}
...
static void sierra_net_defer_kevent(struct usbnet *dev, int work)
{
	...
	if (!schedule_work(&priv->sierra_net_kevent))
		devdbg(dev, "sierra_net_kevent %d may have been dropped", work);
	else
		devdbg(dev, "sierra_net_kevent %d scheduled", work);
}
...
static int sierra_net_change_mtu(struct net_device *net, int new_mtu)
{
	...
	return sierra_net_get_private(dev)->orig_change_mtu(net, new_mtu);
}

static int is_whitelisted(const u8 ifnum, const struct list *whitelist)
{
	if (whitelist) {
		const u8 *list = whitelist->list;
		int i;

		for (i=0; i<whitelist->listlen; i++) {
			if (list[i] == ifnum)
				return 1;
		}
	}
...
static int sierra_net_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if ((status = usbnet_get_endpoints(dev, intf)) < 0) {
	...
	priv->orig_change_mtu = dev->net->change_mtu;
	dev->net->change_mtu = sierra_net_change_mtu;
...
static int sierra_net_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
	...
	while (likely(skb->len)) {
		...
		if (err) {
			...
			/* dev->stats.rx_errors incremented by caller */;
		...
		gfp_t flags)
{
	...
	if (priv->link_up && carve_ethip_packet(skb, dev) && is_ip(skb)) {
		...
		if (is_dhcp(skb) && (resp = handle_dhcp(skb, dev))) {
...
static const struct sierra_net_info_data sierra_net_info_data_direct_ip = {
	...
	.whitelist = {
		.listlen = ARRAY_SIZE(sierra_net_ifnum_list),
		.list = sierra_net_ifnum_list
	}
};
...
static const struct usb_device_id products [] = {
	...
	.driver_info = (unsigned long) &sierra_net_info_direct_ip},
	//{USB_DEVICE(0x1199, 0x9051), /* AT&T Direct IP LTE modem */
	//.driver_info = (unsigned long) &sierra_net_info_direct_ip},
	{}, /* last item */
};
