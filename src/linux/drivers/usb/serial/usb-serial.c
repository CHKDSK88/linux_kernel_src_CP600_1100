...
extern long usb_generic_ioctl(struct usb_device *dev, unsigned int cmd, unsigned long arg);
...
static int serial_ioctl (struct tty_struct *tty, struct file * file, unsigned int cmd, unsigned long arg)
{
	...
	//SW: call generic USB IOCTL handler
	retval = usb_generic_ioctl(port->serial->dev, cmd, arg);
	if (retval <= 0)
		goto exit;
	//SW: continue to specific USB driver ioctl handler
