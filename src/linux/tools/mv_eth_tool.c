...
void show_usage(int badarg)
{
	...
	" mv_eth_tool -tx_noq <idx> <0 | 1>    Set queuing discipline mode: 			\n"
	"                                      0 - Disable, 1 - Enable 				\n"
	"                                                                                       \n"
	" --- Port/Queue commands ---                                                           \n"
	...
	"   info                                Display switch extended info & statistics                 	\n"
	"											\n"
	" mv_eth_tool -net_en <network> <option>			       		                \n"
	"	Enable/Disable connectivity on networks\n"
	"	NETWORKS: \n"
	"		"NET_DEV_EN_PHY_STR"	- WAN interface\n"
	"		"NET_DEV_EN_SWITCH_STR"	- LAN Switch\n"
	"	OPTIONS:\n"
	"		"NET_DEV_EN_ON_STR"		- Turn network device on\n"
	"		"NET_DEV_EN_OFF_STR"	- Turn network device off\n"
        "\n"	
        );
        exit(badarg);
}
...
static int parse_net_dev(char *_net_dev)
{
    if (!strcmp(_net_dev, NET_DEV_EN_PHY_STR))
    {
		port = NET_DEV_EN_PHY;
		return 0;
    }
    else if (!strcmp(_net_dev, NET_DEV_EN_SWITCH_STR))
    {
		port = NET_DEV_EN_SWITCH;
		return 0;
    }
    fprintf(stderr, "Illegal network: %s\n", _net_dev);
    return 1;
}

static int parse_net_dev_state(char *_status)
{
    if (!strcmp(_status, NET_DEV_EN_ON_STR))
    {
        status = NET_DEV_EN_ON;
        return 0;
    }
    else if (!strcmp(_status, NET_DEV_EN_OFF_STR))
    {
        status = NET_DEV_EN_OFF;
        return 0;
    }
...
static void parse_cmdline(int argc, char **argp)
{
	...
	if (!strcmp(argp[i], "-h")) {
	...
	else if (!strcmp(argp[i], "-net_en")) {
		command = COM_NET_DEV_EN;
		...
		i++;
		if (parse_net_dev(argp[i++]))
		{
			show_usage(i);
		}
		if (parse_net_dev_state(argp[i++]))
		{
			show_usage(i);
		}
	}
...
static int procit(void)
{
	...
	switch (command) {
		...
		case COM_NETDEV_VTU_DEL:
			...
			break;
		case COM_NET_DEV_EN:
			fprintf(mvethproc, NET_DEV_EN_CMD_STRING, NET_DEV_EN_PRINTF_LIST);
			break;
		default:
